...
function createMovieCard(m, mediaType = 'movie', extra = {}) {
  ...
  const poster = m.poster_path
    ...
    : `https://via.placeholder.com/160x240/111111/333333?text=No+Poster`;
  ...
  card.innerHTML = `
    <div class="movie-card-img-wrap">
      <img class="movie-card-img" src="${poster}" alt="${title}" loading="lazy" />
      ${progressHtml}
      <div class="movie-card-overlay">
        ...
        ${rating ? `<div class="card-overlay-meta">★ ${rating}</div>` : ''}
      </div>
    </div>
  ...
  card.addEventListener('click', () => goWatch(m.id, mediaType));
  return card;
}
...
// ============ NAVIGATION ============
window.goWatch = function(id, type) {
